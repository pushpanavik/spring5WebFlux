...
public class UserService implements IUserService {
	...
	@Autowired
	private TopicProcessor<User> forgotUser;
		...
		return monoUser.flatMap(u -> {
			return userRepository.findByEmail(u.getEmail()).map(user -> {
				if(passwordEncoder.matches(u.getPassword(), user.getPassword())) {
					String token=GenerateToken.generateUserToken(u.getFirstname(), u.getLastname(), u.getUserId(), u.getEmail());
				return token;
				}
				else {
					return null;
